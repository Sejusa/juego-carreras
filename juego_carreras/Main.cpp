...
int main() {
	char continuar; //Variable donde almacenaré si el jugador quiere hacer otra partida.
	...
	do
	{
		...
		std::cout << "Bienvenido al juego de carreras, disfruta." << std::endl;
		...
		std::cout << "¿Quieres jugar otra partida? Pulsa (s/n)." << std::endl;
		continuar = controladorInput();
		reiniciarInventarios(); //Reiniciamos los inventarios para que empiecen vacíos.
	} while (continuar != 'N'); //Mientras el jugador quiera seguir jugando.
